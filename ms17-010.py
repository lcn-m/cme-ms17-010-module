...
class CMEModule:
    name = 'ms17-010'
    description = "MS17-010 checker from AutoBlue-pack"
    ...
    def on_login(self, context, connection):
        if check_ms17_010(connection.host):
            context.log.highlight("Vulnerable. Msfconsole > use exploit/windows/smb/ms17_010_eternalblue > set RHOST " + connection.host)
        else:
            context.log.highlight("Not vulnerable")
...
def _setup_login_packet_hook(maxBufferSize):
	...
	if maxBufferSize is not None:
		...
		setattr(smb.NewSMBPacket, "addCommand",
				NewSMBPacket_addCommand_hook_login)
...
class MYSMB(smb.SMB):
	...
	def get_dce_rpc(self, named_pipe):
		...
		), self.get_remote_host(), filename='\\'+named_pipe, smb_connection=smbConn)
	...
	def login_extended(self, user, password, domain='', lmhash='', nthash='', use_ntlmv2=True, maxBufferSize=None):
		_setup_login_packet_hook(maxBufferSize)
		smb.SMB.login_extended(self, user, password,
							   domain, lmhash, nthash, use_ntlmv2)
	...
	def nt_create_andx(self, tid, filename, smb_packet=None, cmd=None, shareAccessMode=smb.FILE_SHARE_READ | smb.FILE_SHARE_WRITE, disposition=smb.FILE_OPEN, accessMask=0x2019f):
		self._last_fid = smb.SMB.nt_create_andx(
			self, tid, filename, smb_packet, cmd, shareAccessMode, disposition, accessMask)
	...
	def do_write_andx_raw_pipe(self, fid, data, mid=None, pid=None, tid=None):
		...
		# can be any. raw named pipe does not use it
		writeAndX['Parameters']['Remaining'] = 12345
		writeAndX['Parameters']['DataLength'] = len(data)
		# WordCount(1), ByteCount(2), Padding(1)
		writeAndX['Parameters']['DataOffset'] = 32 + \
			len(writeAndX['Parameters']) + 1 + 2 + 1
	...
	def send_trans(self, setup, param='', data='', mid=None, maxSetupCount=None, totalParameterCount=None, totalDataCount=None, maxParameterCount=None, maxDataCount=None, pid=None, tid=None, noPad=False):
		self.send_raw(self.create_trans_packet(setup, param, data, mid, maxSetupCount,
					  totalParameterCount, totalDataCount, maxParameterCount, maxDataCount, pid, tid, noPad))
		return self.recvSMB()
	...
	def send_trans2_secondary(self, mid, param='', paramDisplacement=0, data='', dataDisplacement=0, pid=None, tid=None, noPad=False):
		self.send_raw(self.create_trans2_secondary_packet(
			mid, param, paramDisplacement, data, dataDisplacement, pid, tid, noPad))
	...
	def send_nt_trans(self, function, setup='', param='', data='', mid=None, maxSetupCount=None, totalParameterCount=None, totalDataCount=None, maxParameterCount=None, maxDataCount=None, pid=None, tid=None, noPad=False):
		self.send_raw(self.create_nt_trans_packet(function, setup, param, data, mid, maxSetupCount,
					  totalParameterCount, totalDataCount, maxParameterCount, maxDataCount, pid, tid, noPad))
		return self.recvSMB()
